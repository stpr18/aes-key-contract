...
static unsigned char aes_rcon_[256] =
{
...
namespace test
{
	...
	{
		for (int i = 0; i < 256; ++i)
			if (Aes::inv_sbox(i) != aes_inv_sbox_[i])
				throw;
	}
	...
	void aes_make_rand_key(uint32_t key[])
	{
		...
		for (int i = 0; i < Aes::SECRET_KEY_SIZE; ++i)
			key[i] = dis(mt);
	}

	void aes_key_contraction(uint32_t key[])
	{
		uint32_t w[Aes::ROUND_KEY_SIZE];
		Aes::key_expansion(key, w);

		uint32_t key2[Aes::SECRET_KEY_SIZE];
		Aes::key_contraction(w + 40, 10, key2);
		for (int i = 0; i < Aes::SECRET_KEY_SIZE; ++i)
			if (key[i] != key2[i])
				throw;
	}
